...
**What cardpack ships after EPIC-04** (nothing landed at `1c14440`): `Ordinal` / `Codebook<D>` (a total `Card ↔ 0..V` bijection per deck, over the deduplicated vocabulary), `CANON_V1` canonical pile bytes, `Permutation` with canonical bytes, `SlotId`, the non-generic `SlotPile`, `Revealed<D>` with `reveal` / `reveal_with`, the `Seal<D>` adapter, and the `seal_roundtrip` conformance helper. Its design rule (EPIC-04 decision 2): **the kernel holds a card's slot, its order, and its value once revealed — never ciphertext, never a scheme type parameter.**

**The consumer this document uses as its worked example — `pkmental`** (`../pkmental`, `main` @ `ac72bc1`): Barnett–Smart threshold ElGamal over Pallas, Sako–Kilian cut-and-choose shuffle proofs, DLEQ reveal tokens, an Ed25519-signed hash-chained event log. The *players* hold and shuffle the masked deck with proofs; cards travel only as curve points (`pkmental/src/wire.rs:11`). The single thing it needs from a card library is a total `Card ↔ 0..52` bijection, which it currently builds itself: `point_for_index(i) = G · (i + 1)` (`pkmental/src/encode.rs:33-35`) over two `OnceLock<HashMap>`s scanned from a 52-entry array (`encode.rs:38-60`), with `expect("every Card is in DECK_ARRAY")` on the encode path (`encode.rs:73-81`). Its crypto seam is `trait CardCrypto` (`pkmental/src/crypto/mod.rs:52`): `MaskedCard: Clone + Eq` (`:62`), `RevealToken` (the untrusted wire form), `VerifiedToken` (mintable only via `verify_reveal_token`), and `MpError` (`:33`, `Clone + Debug + Eq + thiserror::Error`, `&'static str`/`usize` payloads, with `StillMasked` and `BadProof`). `pkmental` does not depend on cardpack today.
...
| Worked consumer example (this doc) | Planned |
| "What a consumer must handle" list (this doc) | Planned |
| `seal_roundtrip` exported under `seal-test-double` (EPIC-04 Story 5) | Planned |
| Doc-test that a `std` crate over `Standard52` can satisfy the helper's bounds | Planned |
...
| `SlotId(u16)`; `SlotPile` holds **only** slots and has no method mentioning `Card` | EPIC-04 Story 4 | Invariant |
...
| `seal_roundtrip<D, S>` conformance helper under `seal-test-double`, bounds `D: Decked<D> + Default + Ord + Copy + Hash` | EPIC-04 Story 5 | Runs unchanged against any backend |
...
|---|---|---|
| `Sealed` | `MaskedCard` (`pkmental/src/crypto/mod.rs:62`) | `Clone + Eq` already; `Debug` may need a wrapper |
...
| `Error` | `MpError` (`mod.rs:33`) | `Send + Sync + 'static` holds — payloads are `&'static str`/`usize` |
...
- [ ] **S1.** The promises table, checked line-by-line against the EPIC-04 API sketches at write time
- [ ] **S2.** The worked example, checked against `pkmental/src/crypto/mod.rs:52-130` and `encode.rs:33-60` at write time (illustration only; cite what is there)
- [ ] **S3.** A doc-test under `seal-test-double` showing `seal_roundtrip` called from a `std`-style harness over `Standard52` with a hand-rolled `Seal` impl — proof the bounds are satisfiable from outside the crate
- [ ] **S4.** Cross-link this doc from `src/seal/mod.rs` as "the surface protocol crates build on"
