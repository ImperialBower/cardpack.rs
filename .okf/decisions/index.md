...
* [The YAML envelope adds a format — it does not replace the legacy sequence reader](yaml-envelope-format.md) - DeckYaml's reader sniffs document shape and supports both the envelope and the legacy bare card sequence; deleting the legacy path passes every test in the new format and silently breaks Razz at build time.
